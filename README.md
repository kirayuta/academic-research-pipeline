...
- **分块**：整篇丢给 AI 只会得到笼统印象。拆成 5 个 ≤4000 token 的章节块，逐块精审，无处偷懒。
- **多轮迭代**：单次审稿只发现表面问题。R1 找问题 → R2 改并验证 → R3 攻击假设 → R4 审数据 → R5 终审润色，连续 3 轮无 P0 才通过。
- **角色轮换**：同一 prompt 反复审同一段文字，第 3 轮开始重复。5 种角色（导师 / 魔鬼代言人 / 统计审计 / 终审编辑）各有独立检查清单，互不干扰。
